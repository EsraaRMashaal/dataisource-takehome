"""
Module: app.services.extraction_engine

Rule-based spaCy extraction: document type detection, metadata,
keywords, entities, commercial notes, and confidence scoring.

All matching is done with tokenization-only spaCy (no heavy model).
Patterns are compiled once at import time and reused for every document.

Consumed exclusively by extraction_service.analyze().
"""
...
from app.services.extraction_models import EntityItem, KeywordItem
...
def extract_keywords(doc: spacy.tokens.Doc, text: str) -> list[KeywordItem]:
    ...
    for match_id, start, end in matches:
        ...
        if norm not in seen:
            seen.add(norm)
            items.append(KeywordItem(keyword=kw, normalized_keyword=norm,
                                     score=score, source_method=method))
    return items
    ...
    for match_id, _start, end in _entity_matcher(doc):
        ...
        items.append(EntityItem(
            ...
            confidence=compute_confidence(
                etype, value, _ENTITY_PATTERN_SCORES[etype], start_char, text
            ),
            quantity_value=qty,
            unit=unit,
